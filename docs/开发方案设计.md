...
- **通知形式**：飞书自定义机器人卡片消息，字段与内容严格参照 `docs/场景用例.md`。
...
- **Header**：`交易对 {symbol}`，模板颜色根据状态（例如成交=绿色、取消=红色、创建=蓝色，可选）。
- **主体元素**（顺序固定，以保障视觉一致）：
  1. 状态行：`状态: {stateLabel}`。
  2. 字段组 1：`Size: {size}` 与（如存在）`累计成交量: {cumulativeQuantity}`。
  3. 字段组 2：根据场景选择显示 `平均成交价格` 或 `价格`：
     - 市价单成交 → `平均成交价格: {displayPrice}`。
     - `SL/TP` 创建/取消 → `价格: {displayPrice}`（即挂单价）。
  4. 通知时间行：`通知时间: {notifyTimeFormatted}`（使用 `dayjs().format('YYYY-MM-DD HH:mm:ss')` + 时区）。
