...
- **累计成交金额**：优先取 `stateTracker` 累加的 `cumulativeQuote`（`Σ lastPrice × lastQuantity`）；若事件提供 `cummulativeQuoteQty` 也可回退使用。
...
- **Header**：采用聚合生成的 `title`（如 `BTCUSDT-反弹1/4减仓30%` / `BTCUSDT-跟踪交易止损`）。模板颜色仍依据状态（创建=蓝色、成交=绿色、部分成交=橙色、取消=红色）。
- **主体元素**（顺序固定，以保障视觉一致）：
  1. 状态行：`状态: {stateLabel}`。
...
| `FEISHU_BOT1_WEBHOOK_URL` | BOT_1（订单创建/取消/过期通知）webhook | 必填 |
| `FEISHU_BOT2_WEBHOOK_URL` | BOT_2（订单成交通知）webhook | 必填 |
| `POSITION_ALERT_WEBHOOK_URL` | BOT_3（持仓校验告警）webhook | 必填 |
| `AGGREGATION_WINDOW_MS` | 聚合窗口毫秒数（保留，可忽略） | `10000` |
...
- **配置示例（.env）**：
  ...
  FEISHU_BOT1_WEBHOOK_URL=https://open.feishu.cn/open-apis/bot/v2/hook/bot1
  FEISHU_BOT2_WEBHOOK_URL=https://open.feishu.cn/open-apis/bot/v2/hook/bot2
  POSITION_ALERT_WEBHOOK_URL=https://open.feishu.cn/open-apis/bot/v2/hook/bot3
  AGGREGATION_WINDOW_MS=10000
  LISTEN_KEY_KEEP_ALIVE_MS=1500000
  LOG_LEVEL=info
  MAX_RETRY=3
  POSITION_VALIDATION_INTERVAL_MS=30000
  ```
